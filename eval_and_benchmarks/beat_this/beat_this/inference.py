...
class Audio2Frames(Spect2Frames):
    ...
    def __init__(self, checkpoint_path="final0", device="cpu", float16=False,
                 feature: str = "logMel", fps: int = 50, fft_size: int = 1024, sone_bands: int = 24):
        super().__init__(checkpoint_path, device, float16)
        self.feature = feature
        self.fps = int(fps)
        self.fft_size = int(fft_size)
        self.sone_bands = int(sone_bands)
        # Build appropriate feature extractor
        if self.feature.lower() == "logmel":
            hop = int(round(22050 / float(self.fps)))
            self.spect = LogMelSpect(sample_rate=22050, n_fft=self.fft_size, hop_length=hop, device=self.device)
        elif self.feature.lower() == "sone":
            self.spect = PsychoSone(sample_rate=22050, fft_size=self.fft_size, fps=self.fps, bark_bands=self.sone_bands, device=self.device)
        else:
...
class Audio2Beats(Audio2Frames):
    ...
    def __init__(
        ...
        super().__init__(checkpoint_path, device, float16, feature=feature, fps=fps, fft_size=fft_size, sone_bands=sone_bands)
